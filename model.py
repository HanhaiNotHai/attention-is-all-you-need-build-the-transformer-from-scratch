...
def run_transformer_forward(
    ...
    if 'token_embedding' in model_params:
        ...
        model_params['token_embedding'] = tgt_embedding.clone()
...
def init_embedding_and_projection_parameters(vocab_size, d_model, tie_weights=True):
    ...
    tgt_embedding = torch.randn(vocab_size, d_model, requires_grad=True)
    output_projection = (
        tgt_embedding if tie_weights else torch.randn(vocab_size, d_model, requires_grad=True)
    )
    return {
...
def collect_model_parameters_into_list(
    encoder_layer_params: list[dict[str, Tensor]],
    decoder_layer_params: list[dict[str, Tensor]],
    embedding_params: dict[str, Tensor],
):
...
def build_uniform_smoothing_distribution(shape: tuple[int, ...], vocab_size: int, epsilon: float):
...
def zero_pad_column_and_pad_token_rows(
    smoothed_distribution: Tensor, gold_token_ids: Tensor, pad_id: int
):
    '''zero the pad column and the rows where the gold token equals pad_id'''

    smoothed_distribution[..., pad_id] = 0
    smoothed_distribution[gold_token_ids == pad_id] = 0
...
def compute_label_smoothed_kl_loss(log_probabilities: Tensor, smoothed_distribution: Tensor):
...
def average_loss_over_non_pad_tokens(total_loss: Tensor, gold_token_ids: Tensor, pad_id: int):
    '''divide total_loss by the count of non-pad tokens in gold_token_ids'''

    return total_loss / max(torch.sum(gold_token_ids != pad_id), 1)
...
def compute_token_accuracy_ignoring_pad(
    log_probabilities: Tensor, gold_token_ids: Tensor, pad_id: int
):
...
def initialize_adam_optimizer_state(parameter_list: list[Tensor]):
    """Allocate Adam m, v zero buffers and a step counter t=0."""

    def zero_buffers():
        return list(map(torch.zeros_like, parameter_list))
...
def apply_adam_bias_correction(m_t: Tensor, v_t: Tensor, beta1: float, beta2: float, step: int):
...
def apply_adam_step_to_all_parameters(
    parameter_list: list[Tensor],
    optimizer_state: dict[str, list[Tensor] | int],
    learning_rate: float,
    beta1: float = 0.9,
    beta2: float = 0.98,
    epsilon: float = 1e-9,
    ...
    for p, m, v in zip(parameter_list, optimizer_state['m'], optimizer_state['v']):
        if p.grad is None:
            continue
        m[:] = update_adam_first_moment(m, p.grad, beta1)
        v[:] = update_adam_second_moment(v, p.grad, beta2)
        m_hat, v_hat = apply_adam_bias_correction(m, v, beta1, beta2, t)
        with torch.inference_mode():
            p.data.sub_(learning_rate * m_hat / (torch.sqrt(v_hat) + epsilon))
